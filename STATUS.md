...
**Owner:** Ziad Meras. **Last updated:** July 19, 2026.
...
- Service configured: Root Directory `backend`, Build `yarn install && yarn build` **+ `yarn db:migrate` appended by the founder (2026-07-19)** so committed Drizzle migrations apply on every deploy, Start `yarn start`, free tier
- **UNVERIFIED (2026-07-19):** whether a deploy has run since PR #7 merged + the Build Command change — i.e. whether the `assessment_sessions` table actually exists in production yet. See §9 open item for the check + fix procedure.
...
- **Evidence database — batch 1 seeded AND loaded into the production Neon DB.** PR #3 merged; the founder ran `npm run db:seed` against production and **verified row counts: 12 sources, 5 compounds, 11 dose_records, 1 bioavailability_record, 1 interaction_record, 7 scoring_parameters.** Compounds: NMN, NR, Resveratrol, Berberine, TMG. NOT yet founder-reviewed (`extractionStatus = ai_extracted`; rationales end "— AI-extracted, pending founder review") — that review is the top open item in §9.
...
- **Assessment session storage — durable, merged (PR #7, July 19, 2026):** the in-memory assessment store is replaced by a Postgres `assessment_sessions` table (session_id PK reusing the existing `assessment_id`, jsonb intake data, 48h `expires_at`; report derived on read, never stored). Anonymous by design — no accounts, no identity; evidence tables untouched. Cleanup without cron (Render free tier): lazy delete-on-read + opportunistic sweep-on-create, both best-effort. Failure behavior is honest: write failure → `503 session_store_unavailable`, missing/expired → `404 session_not_found`, and the frontend shows a clear retry/expired message instead of silently substituting sample data. Migration `0001_assessment_sessions` is additive with IF-NOT-EXISTS guards (safe to re-run). 47 backend tests green. **Caveat: not yet verified applied in production Neon** — see §9.
- **Not yet built / pending:** founder review of the batch-1 evidence (flip `ai_extracted` → `human_reviewed`); more evidence compounds beyond the first 5; the **Start section is intentionally empty** — the recommendation/affiliate engine (firewalled) isn't built; **cross-compound pathway redundancy detection** (NMN+NR) — only same-compound duplicates are flagged today; the **LLM extractor is not the default** (heuristic is) — a founder decision; real legal copy for the 12 routed pages; auth; durable persistence of richer user-side tables (`user_profiles`/`user_stack_items` etc. — anonymous 48h sessions are done via PR #7, accounts/identity are not).
...
- [~] **Evidence data entry — batch 1 SEEDED, LOADED TO PRODUCTION, not yet founder-reviewed.** PR #3 merged; the founder ran `npm run db:seed` against the live Neon DB and verified the row counts (12 sources / 5 compounds / 11 dose_records / 1 bioavailability_record / 1 interaction_record / 7 scoring_parameters). Compounds: **NMN, NR, Resveratrol, Berberine, TMG** (covers all three schema categories: nad_precursor, longevity_compound, methylation). Contents: **12 web-verified real sources**; dose_records per compound — NMN 3, NR 2, Resveratrol 2, Berberine 2, TMG 2 (11 total, includes genuine `null_no_effect` results: Dollerup NR, Yoshino-2012 resveratrol); 1 NMN bioavailability_record; 1 **NMN+NR `redundant_pathway`** interaction_record; **7 scoring_parameters**. **Evidence-tier distribution: A×1 (Berberine), B×4 (NMN metabolic + NMN training + NR + TMG methylation), C×2 (Resveratrol, TMG ergogenic)** — genuine spread, not five identical results. The real scoring engine over this data produces differentiated sub-scores (Berberine 100 · NR 80 · TMG 66.7 · NMN 60 · Resveratrol 50), with the evidence ceiling visibly capping well-dosed weak-evidence compounds. **Verification caveat:** existence + metadata confirmed via web search across multiple independent results (PubMed/PMC/publisher); direct DOI resolution was blocked by the environment proxy (403), so DOIs were assembled from search-confirmed metadata, not fetched byte-for-byte. **This data is LIVE-usable but NOT founder-reviewed:** `extractionStatus = ai_extracted` on every source, and every `evidenceTierRationale` ends "— AI-extracted, pending founder review." (deliberate, disclosed shortcut, founder decision 2026-07-12).
  - [ ] **TOP OPEN ITEM — founder sign-off on batch-1 evidence:** the data is live and scoring against it, but it is NOT yet human-reviewed. Verify each of the 12 sources against its primary (doses/n/outcomes, and the 4 DOIs the founder already spot-checked resolve), correct anything, then flip `extractionStatus` → `human_reviewed` and drop the "pending founder review" suffix from the rationales. Only then is the live scoring formally review-backed rather than AI-extracted. (`npm run db:seed` already run against production; `npm run db:counts` re-verifies row counts read-only.)
- [ ] **VERIFY — `assessment_sessions` migration applied in production (PR #7 follow-up).** The code is merged and `yarn db:migrate` is in Render's Build Command, but it only runs on a deploy, and no deploy is confirmed since PR #7 merged (2026-07-19 ~16:51 UTC). **Check (either works):** (a) Render dashboard → service → Events: is there a successful deploy after that time? (b) Neon console → SQL editor: `SELECT to_regclass('public.assessment_sessions');` — non-null = table exists. **Live smoke test:** `curl -X POST https://thrive-trilogy-stack-builder.onrender.com/assessment -H 'Content-Type: application/json' -d '{"stack_items":[]}'` → `201 {assessment_id}` = new code + table working; `503 session_store_unavailable` = new code deployed but table missing (migration didn't run); and `GET /assessment/<bogus-id>/preview` returning `assessment_not_found` (vs `session_not_found`) = **old code still deployed**. **Fix if not applied:** Render → Manual Deploy → "Deploy latest commit" (Build Command changes and merged code both only take effect on a deploy). The migration is additive/guarded, safe to re-run on every future deploy.
...
- [ ] **Finalize legal copy** for the 12 routed pages — *draft* copy now exists (adapted from the root site per `CLAIMS_COMPLIANCE.md` §5a; content in `frontend/src/lib/legal-content.ts`, each page shows a "Draft for founder review" banner). Still needs founder facts + attorney review before launch. Specific flags surfaced in-page and to resolve:
  - [ ] **Privacy Policy (highest priority):** (a) which AI/LLM provider processes the free-text stack entry, and whether that provider retains entries or trains on them; (b) ~~whether any assessment data is stored server-side once an email is captured, and for how long~~ — **partially answered by PR #7 (2026-07-19):** assessment intake data IS now stored server-side, anonymously (no email/identity attached), hard-capped at 48 hours (`TECH_DOCS.md` §1b) — the Privacy Policy draft must state this; what happens at/after email capture remains undefined (email capture isn't wired to storage); (c) whether `USER_LAB_RESULT` / `USER_FEEDBACK` (schema-supported per `TECH_DOCS.md` §1 but not collected in V1) will be collected later; (d) the email provider and analytics vendor; (e) legal basis / disclosures for target regions (GDPR/UK if applicable)
...
0. **Verify the PR #7 migration is applied in production** (§9 — quick check: Render Events for a post-merge deploy, or the smoke-test curl; fix: Manual Deploy → "Deploy latest commit"). Until then, live assessments may still be running on the old in-memory store.
1. **Founder review of the batch-1 evidence** (§9 top item) — the live scores are AI-extracted until each source is verified and flipped to `human_reviewed`. This is the one thing standing between "working demo" and "trustworthy for real users."
