...
- [~] **Evidence data entry — batch 1 SEEDED, LOADED TO PRODUCTION, not yet founder-reviewed.** PR #3 merged; the founder ran `npm run db:seed` against the live Neon DB and verified the row counts (12 sources / 5 compounds / 11 dose_records / 1 bioavailability_record / 1 interaction_record / 7 scoring_parameters). Compounds: **NMN, NR, Resveratrol, Berberine, TMG** (covers all three schema categories: nad_precursor, longevity_compound, methylation). Contents: **12 web-verified real sources**; dose_records per compound — NMN 3, NR 2, Resveratrol 2, Berberine 2, TMG 2 (11 total, includes genuine `null_no_effect` results: Dollerup NR, Yoshino-2012 resveratrol); 1 NMN bioavailability_record; 1 **NMN+NR `redundant_pathway`** interaction_record; **7 scoring_parameters**. **Evidence-tier distribution: A×1 (Berberine), B×4 (NMN metabolic + NMN training + NR + TMG methylation), C×2 (Resveratrol, TMG ergogenic)** — genuine spread, not five identical results. The real scoring engine over this data produces differentiated sub-scores (Berberine 100 · NR 80 · TMG 66.7 · NMN 60 · Resveratrol 50), with the evidence ceiling visibly capping well-dosed weak-evidence compounds. **Verification caveat:** existence + metadata confirmed via web search across multiple independent results (PubMed/PMC/publisher); direct DOI resolution was blocked by the environment proxy (403), so DOIs were assembled from search-confirmed metadata, not fetched byte-for-byte. **This data is LIVE-usable but NOT founder-reviewed:** `extractionStatus = ai_extracted` on every source, and every `evidenceTierRationale` ends "— AI-extracted, pending founder review." (deliberate, disclosed shortcut, founder decision 2026-07-12).
  - [~] **TOP OPEN ITEM — founder sign-off on batch-1 evidence: IN PROGRESS (2026-07-20), NOT complete.** The data is live and scoring against it, but it is NOT yet human-reviewed — nothing has been flipped to `human_reviewed` and no rationale suffixes dropped. **Review state of the 12 sources:**
    - **7 founder-confirmed accurate:** Yoshino 2021, Martens 2018, Yin 2008, Lan 2015, Liao 2021, Irie 2020, Dollerup 2018.
    - **1 correction applied (2026-07-20):** McRae 2013 (TMG) citation was wrong (Nutr Res 2013;33(2):159–165 → correct **J Chiropr Med 2013;12(1):20–25, doi:10.1016/j.jcm.2012.11.001**); PMC link + underlying science unchanged. Also Dollerup 2018 (NR) sample size null → **n=40** (this correction is why Dollerup is now in the confirmed-7).
    - **4 still unverified:** Timmers 2011, Yoshino 2012, Hoffman 2009, Covarrubias 2021.
    - **How the corrections were applied:** idempotent update script `backend/src/db/corrections/2026-07-20-batch1.ts` (`npm run db:correct:batch1`), NOT a manual DB edit and NOT a re-seed (seed uses ON CONFLICT DO NOTHING and won't UPDATE existing rows); `seed-data.ts` mirrors both values so a fresh re-seed reproduces them. Row counts unchanged (both UPDATEs). **Founder still needs to run `npm run db:correct:batch1` against production** (like `db:seed`) to apply them to the live DB.
    - **Still to do before this item closes:** verify the remaining 4 sources; fill the 3 still-null sample sizes (Yoshino 2012, McRae, Hoffman); resolve the assembled DOIs; decide the recommended-range-vs-studied-dose policy (§ packet); then flip all 12 `extractionStatus` → `human_reviewed`, drop the suffixes, and re-truth the About / How-We-Review "only human-reviewed records feed the score" copy. Read-only re-check anytime with `npm run db:counts`.
    - Review packet (per-compound, with the corrections and remaining flags) delivered to the founder 2026-07-19, updated 2026-07-20.
