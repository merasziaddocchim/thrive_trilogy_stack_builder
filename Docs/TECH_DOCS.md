...
| 2026-07-19 | Google Analytics functional opt-out: GA is **planned, not implemented** — it has never been tagged into this app. **Corrected 2026-08-05:** this row previously said "nothing exists in this repo either", which stopped being true when **Vercel Web Analytics was installed on 2026-08-04 (PR #32)** — a different vendor, which this row's GA-specific wording did not catch. Per `CLAIMS_COMPLIANCE.md` §5b (owning rule), that rule is **now vendor-agnostic**: any analytics, advertising or visitor-tracking technology requires (a) the Privacy/Cookie/Do-Not-Sell pages updated first, and (b) where it sets a cookie or a persistent identifier, or discloses personal information for cross-context behavioral advertising, a functional opt-out plus a GPC-honoring decision in the same change. Vercel Web Analytics is cookieless, so only (a) applied | **Open — future engineering item** for GA, paired with the founder's GA ad-features decision (Google Signals / ads personalization on or off), due at or before tagging |
