...
| 2026-07-19 | Google Analytics functional opt-out: GA is in use at the site level but has no tag/integration in this repo yet. Per `CLAIMS_COMPLIANCE.md` §5b (owning rule), GA must not be tagged into the app frontend without a functional opt-out (actually stopping the GA data flow, e.g. consent-gated tag loading / `ga-disable-<ID>`) and a GPC-honoring decision shipping in the same change | **Open — engineering follow-up**, blocked on the founder's GA ad-features decision (Google Signals / ads personalization on or off) |
