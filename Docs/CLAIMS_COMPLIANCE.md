...
- **Disclose the actual role AI plays in any user-facing report — and only that role.** This cuts both ways: overstating AI's involvement is the same category of deceptive gap as overstating its capability, and it is the error this project actually made. The earlier prescribed sentence here ("This report was generated using AI, based on our reviewed research database…") was **withdrawn on 2026-07-24 as an overclaim**: the report is not AI-generated. Three distinct things happen, and the disclosure must keep them distinct:
  ...
  **OPEN ITEM — the Confirm screen is a fourth, separate copy (recorded 2026-07-29).** `frontend/src/components/assessment/ConfirmStep.tsx` does not import `AI_ROLE_NOTE`; it carries its own hardcoded sentence describing intake as automated text-matching and not an AI model. That sentence is **accurate today** and consistent with item 3 above — this is not a live disclosure defect. But it is outside the single-sourcing that the rest of this note relies on, so it can drift independently, which is exactly the failure mode that produced the withdrawn overclaim. Consolidating it is a code change and a founder decision (the Confirm screen describes intake specifically, not the whole pipeline, so the full `AI_ROLE_NOTE` may not be the right text for that surface — that judgment has not been made). Until it is resolved, **any edit to the AI-role wording in this section must be applied to `ConfirmStep.tsx` as well as to the constant.**
